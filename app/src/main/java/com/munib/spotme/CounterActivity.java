...
public class CounterActivity extends BaseActivity {
    ...
    EditText interest_rate,amount,selected_username;
    ...
    Spinner duration;
    Boolean selected=false,signed=false,universal=false;
    ...
    TextView previous_amount,previous_interest,previous_total;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        offersData= (OffersModel) getIntent().getSerializableExtra("data");
      ...
      //  selected_username=(EditText) findViewById(R.id.selectedUsername);
        ...
        if(offersData.getType().equals("lend"))
        {
            DatabaseReference ref = FirebaseDatabase.getInstance().getReference().child("users");
            ref.child(offersData.getLender()).addValueEventListener(new ValueEventListener() {
                @Override
...
//                        selected_username.setText(selectedUser.getUsername());
                        selected_user_token=selectedUser.getDevice_token();
                    } else {
            ...
            DatabaseReference ref = FirebaseDatabase.getInstance().getReference().child("users");
            ref.child(offersData.getBorrower()).addValueEventListener(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
                    if (dataSnapshot.exists()) {
                        UserModel user = dataSnapshot.getValue(UserModel.class);
...
//                        selected_username.setText(selectedUser.getUsername());
        ...
        previous_amount=findViewById(R.id.previous_amount);
        previous_interest=findViewById(R.id.previous_interest);
        previous_total=findViewById(R.id.previous_total);
        previous_amount.setText("$ "+offersData.getAmount()+".0");
        previous_interest.setText(offersData.getInterestRate()+"%");
        previous_total.setText("$ "+offersData.getAmountAfterInterest()+".0");
        ...
        proposed_payment_card.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                LayoutInflater factory = LayoutInflater.from(CounterActivity.this);
                final View deleteDialogView = factory.inflate(R.layout.dialog_proposed_payments, null);
                BottomSheetDialog deleteDialog = new BottomSheetDialog(CounterActivity.this,R.style.BottomSheetDialog);
                ...
                rv1.setLayoutManager(new LinearLayoutManager(CounterActivity.this));

                ProposedMoneyAdapter adapter1=new ProposedMoneyAdapter(CounterActivity.this,proposedMoneyArrayList);
                  ...
                  if(offersData.getType().equals("lend")) {
                      url=
                              "<p><strong>SPOTME LOAN AGREEMENT</strong></p>\n" +
                                      ...
                                      "<p>THIS LOAN AGREEMENT (the &ldquo;Agreement&rdquo;), is made and entered into by and between <u><b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+"(@"+selectedUser.getUsername()+") "+selectedUser.getName()+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b></u> (hereinafter, known as &ldquo;Lender&rdquo;) and  <u><b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+"(@"+currentUserData.getUsername()+") "+currentUserData.getName()+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b></u>, (hereinafter, known as &ldquo;Borrower&rdquo;). Both the Lender and the Borrower shall collectively be referred to herein as &ldquo;the Parties&rdquo;. This Agreement governs the process by which you may make a request or requests for a loan from other users of SpotMe through the website spotme.net or the spotme app, including any subdomains thereof, or other application channels offered by us (collectively, the \"Site\"). If you make a loan request, and if that request results in a loan that is approved and funded by us, then your loan will be governed by the terms of the Loan Agreement.</p>\n" +
                                      "<ol>\n" +
                                      ...
                                      +"<div class='parent'>\n\n\n" +
                                      "  <div style='display:inline-block'><center><img src='{IMAGE_PLACEHOLDER}'></br><u>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+selectedUser.getName()+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</u></center></div>\n" +
                                      "  <div style='display:inline-block'><center><img src='{IMAGE_PLACEHOLDER1}'></br><u>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+currentUserData.getName()+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</u></center></div>\n" +
                                      "</div>";
                  }else{
                      url =
                              "<p><strong>SPOTME LOAN AGREEMENT</strong></p>\n" +
                  ...
                  deleteDialogView.findViewById(R.id.continue_btn).setOnClickListener(new View.OnClickListener() {
                      ...
                      public void onClick(View view) {
                          if(!signed)
                          {
                              ...
                              if(offersData.getType().equals("lend"))
                              {
                                  url = url.replace("{IMAGE_PLACEHOLDER1}", image);
                              }else{
                                  url = url.replace("{IMAGE_PLACEHOLDER}", image);
                              }
                  ...
                  if(offersData.getType().equals("lend")) {
                      ...
                      new Timer().schedule(new TimerTask() {
                          ...
                          public void run() {
                              ...
                              for (int i = 0; i < proposedMoneyArrayList.size(); i++) {
                                  ...
                                  database.getReference("payments").child(selectedUser.getUid() + "_" + currentUserData.getUid() + "_" + timez).child(model.getIndex() + "").setValue(model);
                              }

                              OffersModel userModel = new OffersModel("borrow", amount.getText().toString(), amountAfterLoan_value + "", interest_rate.getText().toString(), duration.getSelectedItem().toString(), 0, new Date().getTime() + "", true, true, url, selectedUser.getUid(), currentUserData.getUid());
                              database.getReference("requests").child(selectedUser.getUid() + "_" + currentUserData.getUid() + "_" + timez).setValue(userModel);
                              database.getReference("offers").child(offersData.getOffer_uid()).child("status").setValue(-2);

                              runOnUiThread(new Runnable() {
                                  public void run() {
                                      if (selected_user_token != null) {
                                          sendNotification(currentUserData.getName() + " has made a counter offer to borrow $" + amount.getText().toString()+" from you", selected_user_token);
                                          database.getReference("notifications").child(selectedUser.getUid()).push().child("notification").setValue(currentUserData.getName() + "  has made a counter offer to borrow $" + amount.getText().toString()+" from you");

                                      }

                                      Toast.makeText(CounterActivity.this, "Counter Offer Sent Successfully!", Toast.LENGTH_LONG).show();
                                      hideProgress();
                                  }
                              });
                      ...
                      new Timer().schedule(new TimerTask() {
                          ...
                          public void run() {
                              ...
                              if(universal){
                                 // database.getReference("universal_requests").child(offersData.getOffer_uid()).child("status").setValue(-1);
                              }else
                                 database.getReference("requests").child(offersData.getOffer_uid()).child("status").setValue(-2);

                              runOnUiThread(new Runnable() {
                                  public void run() {
                                      if (selected_user_token != null) {
                                          sendNotification(currentUserData.getName() + " has made a counter offer to lend $" + amount.getText().toString()+" to you", selected_user_token);
                                          database.getReference("notifications").child(selectedUser.getUid()).push().child("notification").setValue(currentUserData.getName() + " has made a counter offer to lend $" + amount.getText().toString()+" to you");
